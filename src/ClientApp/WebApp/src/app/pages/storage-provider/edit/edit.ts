...
import { StorageProviderUpdateDto } from 'src/app/services/admin/models/system-mod/storage-provider-update-dto.model';
import { StorageProviderDetailDto } from 'src/app/services/admin/models/system-mod/storage-provider-detail-dto.model';

@Component({
  selector: 'app-storage-provider-edit',
...
export class StorageProviderEdit implements OnInit {
  ...
  constructor(
    ...
    private dialogRef: MatDialogRef<StorageProviderEdit>,
  ...
  ngOnInit() {
    if (this.id) {
      ...
      this.adminClient.storageProvider.detail(this.id).subscribe({
        next: (res: StorageProviderDetailDto) => {
          this.form.patchValue(res);
          this.updateValidatorsBasedOnCloud(res.isCloud);
  ...
  private setupDynamicValidation() {
    this.isCloud.valueChanges.subscribe(value => {
      this.updateValidatorsBasedOnCloud(value);
    });
  }

  private updateValidatorsBasedOnCloud(isCloudValue: boolean) {
    if (isCloudValue) {
      // 云存储：必填云配置字段
  ...
  submit() {
    ...
    if (!this.id) return;
    this.adminClient.storageProvider.update(this.id, this.form.value as StorageProviderUpdateDto).subscribe(() => this.dialogRef.close(true));
  }
