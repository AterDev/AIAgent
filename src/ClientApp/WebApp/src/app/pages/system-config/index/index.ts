...
import { SystemConfigAdd } from '../add/add';
import { SystemConfigEdit } from '../edit/edit';
import { SystemConfigDetail } from '../detail/detail';

@Component({
  selector: 'app-system-config-index',
  imports: [CommonListModules, CommonFormModules],
  templateUrl: './index.html',
  standalone: true
})
export class SystemConfigIndex implements OnInit {

  i18nKeys = I18N_KEYS;

  filterDto: SystemConfigFilterDto = { pageIndex: 1, pageSize: 10 };
  ...
  loadData(): void {
    this.adminClient.systemConfig.list(this.filterDto as SystemConfigFilterDto).subscribe((res) => {
  ...
  openAdd() {
    const ref = this.dialog.open(SystemConfigAdd, { width: '800px' });
    ref.afterClosed().subscribe((r: boolean) => { if (r) this.loadData(); });
  }

  openEdit(id: string) {
    const ref = this.dialog.open(SystemConfigEdit, { width: '800px', data: { id } });
    ref.afterClosed().subscribe((r: boolean) => { if (r) this.loadData(); });
  }

  openDetail(id: string) {
    this.dialog.open(SystemConfigDetail, { minWidth: '600px', data: { id } });
  }
  ...
  deleteItem(id: string) {
    ...
    ref.afterClosed().subscribe((ok: boolean) => {
      if (ok) { this.adminClient.systemConfig.delete(id).subscribe(() => this.loadData()); }
    });
  }
}
